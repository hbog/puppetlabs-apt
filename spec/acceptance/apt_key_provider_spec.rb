...
PUPPETLABS_EXP_KEY_SHORT_ID    = '4BD6EC30'
PUPPETLABS_EXP_KEY_LONG_ID     = "1054B7A24BD6EC30"
PUPPETLABS_EXP_KEY_DATES       = ":2010-07-10:2016-07-08:"
...
PUPPETLABS_EXP_CHECK_COMMAND   = "#{KEY_CHECK_COMMAND} :#{PUPPETLABS_EXP_KEY_LONG_ID}:"
...
-----END PGP PUBLIC KEY BLOCK----- ",
  ...
  describe 'refresh' do
    before(:each) do
      # delete the Puppet Labs Release Key
      # and install an expired version of the key
      shell("apt-key del #{PUPPETLABS_EXP_KEY_SHORT_ID}",
          :acceptable_exit_codes => [0,1,2])
      pp = <<-EOS
      apt_key { 'puppetlabs':
...
-----END PGP PUBLIC KEY BLOCK-----'
      ...
      EOS
      apply_manifest(pp, :catch_failures => true)
      shell("#{PUPPETLABS_EXP_CHECK_COMMAND}")
    end
    ...
    context 'when refresh => true' do
      it 'updates an expired key' do
         pp = <<-EOS
          ...
          # Key is present
          shell("#{PUPPETLABS_EXP_CHECK_COMMAND}")
          # And has been updated by a new version
          shell("#{PUPPETLABS_EXP_CHECK_COMMAND} | grep #{PUPPETLABS_EXP_KEY_DATES}",
                    :acceptable_exit_codes => [1])
      end
    end
